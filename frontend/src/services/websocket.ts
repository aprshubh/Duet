...
export class WebSocketClient {
  ...
  public sendReaction(emoji: string, userName: string) {
    this.send('ROOM_REACTION', { emoji, userName });
  }

  public sendVideoURL(url: string, sourceType: 'youtube' | 'direct' | 'file', title?: string) {
    this.send('VIDEO_URL_CHANGE', { url, sourceType, title });
  }

  public sendVoiceSignal(targetUserId: string, signalType: 'offer' | 'answer' | 'candidate', data: unknown) {
    this.send('VOICE_SIGNAL', { targetUserId, signalType, data });
  }

  public sendVoiceState(inVoice: boolean, isMuted: boolean) {
    this.send('VOICE_STATE', { inVoice, isMuted });
  }
